#!/usr/bin/env python3
"""Authorized V5.4 repair Candidate B training; no heldout path exists here."""
...
AUTH = PROJECT / 'artifacts/v5_4_pipeline/review/root_v54_repair_training_authorization_20260918.json'
AUTH_SHA = '707df6d29210f1f18152715b4a385a98067781ce136afd126818213a4820c349'
TRAIN = PROJECT / 'artifacts/v5_4_pipeline/data_v3/train_v5_4_v3.jsonl'
TRAIN_SHA = '56d8b5590a87759233fbbd3f5baa4e70796143f17519d92ac21c56c0dfc9dff2'
BASE = PROJECT / 'models/Qwen2.5-1.5B-Instruct'
PEFT = PROJECT / 'output/tcm-qwen-1.5b-v5-3'
OUTPUT = PROJECT / 'output/tcm-qwen-1.5b-v5-4-candidate-b'
RUNTIME = PROJECT / 'tcm_chat_v5.py'
RUNTIME_SHA = 'ff6e7337ed1c54217cbb153473aa417af6680e8d185db3e5bb8364f125c087fe'
ARTIFACT = PROJECT / 'artifacts/v5_4_pipeline/training/candidate_b'
PREFLIGHT = PROJECT / 'artifacts/v5_4_pipeline/training/candidate_b_preflight.json'
PROTECTION = PROJECT / 'artifacts/v5_4_pipeline/review/root_v54_repair_preflight_protection_20260918.json'
...
def preflight()->dict[str,Any]:
 ...
 if auth.get('status')!='ALLOW' or auth.get('scope',{}).get('training')!='ALLOW' or auth.get('scope',{}).get('heldout_access')!='DENY': raise RuntimeError('authorization not valid for candidate training')
 approved=auth.get('approved_training',{})
 if approved.get('training_data')!=str(TRAIN) or approved.get('base_model_adapter')!=str(PEFT) or approved.get('output_dir')!=str(OUTPUT) or approved.get('resume_from_checkpoint') is not False: raise RuntimeError('authorized training inputs differ')
 if auth.get('bindings',{}).get('train_sha256')!=TRAIN_SHA: raise RuntimeError('authorized train hash differs')
 regular_file(PROTECTION,'protection report')
 if sha(PROTECTION)!=PROTECTION_SHA or json.loads(PROTECTION.read_text(encoding='utf-8')).get('status')!='PASS': raise RuntimeError('protected baseline verification mismatch')
 ...
 return {'status':'PASS','candidate':'B','authorization_sha256':AUTH_SHA,'protection_report_sha256':PROTECTION_SHA,'train':str(TRAIN),'train_sha256':TRAIN_SHA,'runtime_sha256':RUNTIME_SHA,'peft_start':str(PEFT),'output':str(OUTPUT),'gpu_free_mib':free,'protected_hash_checks':protected,'hyperparameters':{'epochs':1,'learning_rate':3e-6,'train_batch':1,'eval_batch':1,'gradient_accumulation_steps':8,'max_length':768,'validation_percent':5,'warmup_ratio':0.03,'seed':42,'data_seed':42,'resume_from_checkpoint':False,'logging_steps':1,'eval_steps':25,'save_steps':25},'heldout_access':'DENY'}
...
def weight_manifest()->dict[str,Any]:
 ...
 if not files: raise RuntimeError('Candidate B has no regular files')
 path=ARTIFACT/'candidate_b_weights.sha256';path.write_text(''.join(f'{sha(p)}  {p.relative_to(OUTPUT)}\n' for p in files),encoding='utf-8')
...
def main()->int:
 ...
 run(cmd,ARTIFACT/'train.log');write_new(ARTIFACT/'training_evidence.json',evidence());write_new(ARTIFACT/'candidate_b_weight_manifest.json',weight_manifest());write_new(ARTIFACT/'postflight.json',{'status':'PASS','protected_hash_checks':[check(*i) for i in PROTECTED]});print(json.dumps({'status':'PASS','candidate':'B','artifact':str(ARTIFACT),'output':str(OUTPUT)},ensure_ascii=False));return 0
